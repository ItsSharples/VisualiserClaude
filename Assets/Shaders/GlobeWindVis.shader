	...
	SubShader
	{
		...
		Pass
		{
			...
			float4 frag (v2f i) : SV_Target
			{
				...
				float2 windVector = tex2D(_MainTex, TRANSFORM_TEX(uv, _MainTex)).rg;
				float m = length(windVector) / _WindMax;
				float3 windCol = lerp(_ColA, _ColB, saturate(m));
				float3 overlayCol = tex2D(_OverlayTex, TRANSFORM_TEX(uv, _OverlayTex));

				float3 col = lerp(windCol, overlayCol, _OverlayStr);
				return float4(col, 1);
			}
			ENDCG
		}
